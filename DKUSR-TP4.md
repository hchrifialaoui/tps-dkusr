...
Vous trouverez ci-dessous un exemple d’une ressource de type ***deployment***. Créez un fichier `deployment-kubernetes-app.yaml` (dans le dossier  `~/hakim/deployment/`) et copiez-y le code suivant :
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: kubernetes-app
        image: 653925650847.dkr.ecr.eu-west-1.amazonaws.com/hakim/app:v0.1
        ports:
        - containerPort: 8000
          name: http
```
...
kubernetes-app NodePort   10.97.54.113   <none>        8080:32328/TCP   5s
```
...
Ce TP est maintenant terminé. Procédons au nettoyage :

```sh
dev $ kubectl delete cm cm1 cm2 cm3 cm4 cm5
configmap "cm1" deleted
configmap "cm2" deleted
configmap "cm3" deleted
configmap "cm4" deleted
configmap "cm5" deleted
```
