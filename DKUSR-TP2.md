...
```
La commande liste non pas une mais dix versions (images) de la distribution `alpine` à partir de son dépôt stocké sur le Docker Hub. On remarque que certaines versions correspondent bien aux couches que l'on a téléchargé précédemment grâce à la commande `docker image pull`. On peut donc choisir une image parmi ces dix versions afin de démarrer un conteneur.
...
Veillez à quitter le conteneur dans lequel vous êtes. Puis préparez à travailler dans le répertoire de travail `~/hakim/kubernetes-app/` (le répertoire est déjà présent). Nous allons être amenés à créer plusieurs fichiers dans ce répertoire :
...
Maintenant que nous avons créé notre fichier Dockerfile, il est temps de créer notre fichier docker-compose qui va nous simplifier la construction et le lancement de notre application. Nous allons créer un fichier `docker-compose.yml` (toujours dans  `~/hakim/kubernetes-app/`) avec le contenu suivant :
```yaml
...
Successfully tagged 653925650847.dkr.ecr.eu-west-1.amazonaws.com/hakim/app:v0.1
```
...
```
dev $ docker history 31920ffe5099
